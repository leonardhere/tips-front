...
$(function() {
    ...
    const changeLevelCallback = (level) => {
        const activeRow = $('#buyProductModal .product-table__row--content').not('.hidden');
    ...
    $('#buyProductModal').on('show.bs.modal', function(e) {
        ...
        const minSaleQty = modal.find('#min-sale-qty');
        const productUnit = root.find('.product-card__unit').text();
        const modalType = data['modalClass'];
        const modalContent = $(this).find('.product-table__row--content'+'.'+modalType);
    ...
    function showSliderValue(target) {
        const $target = $(target);
        const root = $target.closest('.sidebar-nav__item');
        const selectedContainer = root.find('.catalog-filter--selected');
        const currentValues = target.noUiSlider.get();

        const minValue = Number($target.find('.js-min-value').val());
        const maxValue = Number($target.find('.js-max-value').val());

        const currentMinValue = Number(currentValues[0]);
        const currentMaxValue = Number(currentValues[1]);
    ...
    function appendListeners() {
        ...
        $('.sidebar-menu .submenu > .sidebar-nav__line > a:not(.sidebar-menu-close)').on('click', function (e) {
          if (window.tablet) {
              e.preventDefault();
        ...
        $('#catalog-nav .sidebar-menu-back').on('click', function (e) {
            if (window.tablet) {
                e.preventDefault();
    ...
    $(document).on('submit', '#catalog-filters', function (e) {
        ...
        $('.js-slider').each(function () {
            ...
            const minValue = Number($this.find('.js-min-value').val());
            const maxValue = Number($this.find('.js-max-value').val());
        ...
        && currentUrl.indexOf('cat_type=grouped') < 0
        && currentUrl.indexOf('cat_type=bundle') < 0) {
    ...
    window.addEventListener("popstate", function(e) {
        if (e.state == 'filter') {// check for 'filter' object
    ...
    $('.custom-control__preview').popover({
        ...
        container: 'body',
        content: function() {
            let imgSrc =  $(this).data('background-image');
