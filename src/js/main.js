$(function() {
    ...
    function showTopMenuLevel0(row) {
        if (!window.tablet) {
            ...
            const heightNav1 = $(row).find('.navbar-1').outerHeight();
    ...
    function showTopMenuLevel1(row) {
        if (!window.tablet) {
            ...
            const heightNav1 = $(row).closest('.navbar-1').outerHeight();
            const heightNav2 = $(row).find('.navbar-2__nav').outerHeight();
            const maxHeight = Math.max(heightNav1 - heightNav2 + INITIAL_INDENT, heightNav2 + INITIAL_INDENT);
    ...
    function showTopMenuLevel2(row) {
        if (!window.tablet) {
            ...
            const heightNav1 = $(row).closest('.navbar-1').outerHeight();
            const heightNav2 = $(row).closest('.navbar-2 .navbar-2__nav').outerHeight();
            const heightNav3 = $(row).find('.navbar-3 .navbar-3__nav').outerHeight();
            const maxHeight = Math.max(heightNav1 - heightNav2 + INITIAL_INDENT, heightNav2 - heightNav3 + INITIAL_INDENT, heightNav3 + INITIAL_INDENT);
    ...
    function fixHeader() {
        ...
        header.removeClass('fixed-head').error(function() {
            return 0;
        }); //if window resized
        $('.site-controls').css('margin-bottom', '');
        var posFromTop = header.offset().top;
        // console.log(posFromTop);
        $(window).on("scroll", function(e) {
            if ($(window).scrollTop() >= posFromTop && !window.tablet) {
                // console.log($(window).scrollTop());
                $('.site-controls').css('margin-bottom', header.outerHeight());
                header.addClass("fixed-head");
                // $('body').css('padding-top', posFromTop);
            } else {
                $('.site-controls').css('margin-bottom', '');
    ...
    const MEDIA_BREAKPOINT_TABLET = 1024;
    const MEDIA_BREAKPOINT_MOBILE = 768;
    const INDENT_DESKTOP = 0;
    const INDENT_MOBILE = 0;
    const INITIAL_INDENT = $(window).width() >= MEDIA_BREAKPOINT_TABLET
        ? INDENT_DESKTOP
        : INDENT_MOBILE;
    ...
    const sideslide = $('[data-toggle=collapse-side]');
    const collapsable = sideslide.attr('data-target');
    sideslide.click((e) => {
    ...
    $('#goods-dropdown').on('show.bs.dropdown', function () {
      $('.overlay').removeClass('hidden');
    });
    $('#goods-dropdown').on('hide.bs.dropdown', function () {
      $('.overlay').addClass('hidden');
    });
    ...
    function setSearchStyle() {
        $('#search-result-mobile .dropdown-search').css('max-height', window.innerHeight
            - $('.navbar-iso').outerHeight()
            - $('#mobile-search-wrap').outerHeight() - 30);
    };
    ...
    $(document).on('click', '.dropdown-search, #mobile-search-wrap, .navbar-main__item--products .dropdown-menu', function(e) {
        e.stopPropagation();
    });
    ...
    $('.sidebar-menu--drop').on('click', function(e) {
        e.stopPropagation();
    });
...
$(function() {
        ...
        responsive: [
            {
                ...
                settings: {
                    ...
                    focusOnSelect: true,
                    vertical: true,
                    asNavFor: '.product-summary-slider-main'
                }
            }, {
                breakpoint: 1199,
                settings: {
        ...
        responsive: [
            {
                ...
                settings: {
                    ...
                    arrows: false,
                    focusOnSelect: true,
                    asNavFor: '#product-summary-slider'
                }
            }, {
                breakpoint: 1199,
                settings: {
    ...
    function isTabActive() {
        return $('#product-add-nav .nav-item.active').length
            ? ''
            : 'active';
    }
    ...
    (function() {
                ...
                if(daysLeft <= 0) {
                  ...
                  $this.countdown(thisDate, function(event) {
                    daysLeft == 1 ?
                      ...
                      : $this.html(event.strftime('%D '+ declOfNum(daysLeft - 1,  ['день', 'дня', 'дней'])));
                  });
                }
            });
        });
        ...
        $('.promo-countdown--main').each(function() {
            ...
            if(minDay <= 0) {
              ...
              $this.countdown(minDate, function(event) {
                minDay == 1 ?
                    $this.html(event.strftime('%H:%M:%S'))
                  : $this.html(event.strftime('%D '+ declOfNum(minDay - 1,  ['день', 'дня', 'дней'])));
              });
        ...
        function getMainPhone($this) {
            return ($this.hasClass('mobile-phone-item-contacts-js')) ?
                $this.attr('href') :
                $('#comagic_phone_1').attr('href');
        }
    })();
});
...
$(function() {
    ...
    $('.smooth-scroll').not('[href="#"]').not('[href="#0"]').click(function(event) {
        if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
            ...
            var topOffset = $('body').outerHeight() > (targetTopOffset - parseFloat($('body').css('padding-top')))
